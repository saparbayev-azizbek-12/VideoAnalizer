...
class DangerZoneState:
    """
    Real-vaqtda BITTA kamera uchun xavfli-hudud monitoring holati.

    Har bir kamera o'zining DangerZoneState'iga ega bo'lishi kerak, chunki
    poligon (va shunga bog'liq sv.PolygonZone/annotator obyektlari) har bir
    kamerada boshqacha bo'lishi mumkin. Og'ir YOLO modelini esa (xotira
    tejash uchun) barcha kameralar bo'lishishi mumkin - shu sabab `model`
    tashqaridan (models_manager.py dan) beriladi, bu yerda qayta yuklanmaydi.
    """
    ...
    def update_polygon(self, polygon: np.ndarray) -> None:
        """Foydalanuvchi GUI orqali zonani qayta belgilaganda chaqiriladi."""
        self.polygon = polygon
        self.zone = sv.PolygonZone(polygon=polygon)
        self.zone_annotator = sv.PolygonZoneAnnotator(zone=self.zone, color=sv.Color.RED, thickness=2)
    ...
    def analyze(self, frame: np.ndarray, conf: float = 0.35) -> tuple[np.ndarray, int, bool]:
        """
        Qaytaradi: (annotatsiyalangan_kadr, hududdagi_odamlar_soni, buzilish_bormi)
        """
