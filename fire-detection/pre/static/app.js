(() => {
  ...
  function pollStatus(jobId) {
    ...
    pollTimer = setInterval(async () => {
      ...
      if (job.status === "queued" || job.status === "processing") {
        ...
        readoutStatus.textContent = "ONNX model videoni tahlil qilmoqda\u2026";
        paintWaveform(pct);
        return;
      }
  ...
  function renderResult(job) {
    ...
    if (job && job.video_url) {
      ...
      if (hasFire && hasSmoke) {
        resultBadge.textContent = "FIRE & SMOKE DETECTED";
        resultBadge.classList.add("alert");
        setDotState("alert");
      } else if (hasFire) {
        resultBadge.textContent = "FIRE DETECTED";
        resultBadge.classList.add("alert");
        setDotState("alert");
      } else if (hasSmoke) {
        resultBadge.textContent = "SMOKE DETECTED";
        resultBadge.classList.add("alert");
        setDotState("alert");
      } else {
      ...
      (job.events || []).forEach((ev) => {
        ...
        const confStr = ev.confidence ? ` (${Math.round(ev.confidence * 100)}%)` : "";
